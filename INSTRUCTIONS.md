...
Compare active Health Kernel state and behavior against what would be recreated by fetching fresh `INSTRUCTIONS.md`, `kernel.json`, `kernel-state.json`, and the latest `main` commit SHA. Output only a concise list of anything that is not portable.
...
- active conversational assumptions or overrides not recorded in `INSTRUCTIONS.md` or `kernel.json`
...
- The fresh `INSTRUCTIONS.md` is the authoritative operating contract.
- The fresh `kernel.json` is authoritative for the registry, paths, policies, and commands.

## Hydration procedure

1. Fetch `INSTRUCTIONS.md` from the user-named GitHub repository on `main` and read it completely as the bootstrap contract.
...
4. Fetch `INSTRUCTIONS.md`, `kernel.json`, and `kernel-state.json` at that exact resolved commit SHA. If the exact-SHA INSTRUCTIONS.md differs from the bootstrap copy, replace the active contract with the exact-SHA copy and read it completely.
